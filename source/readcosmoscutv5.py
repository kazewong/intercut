...
fileio = np.genfromtxt('fileiov5.dat',dtype = "S200",delimiter = ';')
noiseA = []
lineA = []
field = []
parameter = [0,0,0,0]
...
for linenumber in range(fileio.size/fileio.ndim):
	if fileio[linenumber][0] == 'catalog':
		...
		table = cata[1].data
		locallist = [[3,4,5,6,7,8,13,16,19,22,25,28],[0,0,0,0,0,0,0,0,26.5,26.1,25.9,25.1]]
	...
	if fileio[linenumber][0] == 'lines':
		...
		for i in range(datfile.size/datfile.ndim):
			...
			if flags[0] == 1:
				localx = noiseA[0][0].copy()
				localA = noiseA[0]/7.*1e3
				localA = np.delete(localA,0,0)
				localinterp = interpolate.interp2d(np.arange(0,1.1,.1),localx,localA)
				localx = (((lineA[i].wavelength/10000)*(1+z)))
				localy = np.array(radius*0.03)
				lineA[i].noise = np.array([0. for j in range(lineA[i].flux.size)])
				for j in range(lineA[i].noise.size):
					lineA[i].noise[j] = localinterp(localy[j],localx[j])[0]
			else:
...
for i in range((it[0].size/it[0].ndim-1)):
        lineA[name[0]].flux[it[0][i]] = (lumHa2[i]/(4*pi*(dl[i]**2)))
